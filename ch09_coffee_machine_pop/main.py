...
# 자판기 보유량에서 에스프레소 두 잔을 추출했을 떄 resources의 남은 물, 우유, 커피량을 딕셔너리 형태로 보여주시오.
# 로직
...
# 라떼 한 잔을 뽑았을 때 남은 resources를 출력하고, 라떼 금액만큼 profit 더한 결과를 콘솔에 출력하시오.

# 로직
# for key in MENU['카푸치노']['재료']:
#     resources[key] -=  MENU['카푸치노']['재료'][key]
# profit  += MENU['카푸치노']['가격']
# print(resources)
# print(profit)
# 함수 정의 영역
...
def is_resource_enough(order_ingredients):  # order_ingredients = dri nk['재료]
    '''
    DocString : 함수 / 클래스 / 메서드가 어떤 작동을 하는지 ' 사람들에게' 설명하는 기능 주문 받은 음료를 resources에서 재료 차감을 하고 난 후, 음료 만들기가 가능하면 True / 아니면 False
...
def is_transaction_successfull(money_received, drink_cost):
    '''
    process_coins()의 결과값과 음료 가격을 매개 변수로 받아 동전이 가격보다 높으면 True / 아니면 False를 반환하는데 금액이 부족하다고 안내해줘야 합니다. 그리고 True라면 profit에 음료 가격만큼 추가를 해주고, 잔돈을 반환해야 합니다.
...
while is_on:
    ...
    # todo - 1 : choice가 off라면 자판기가 종료되었습니다. 라고 출력함녀서 반복 종료
    if choice == 'off':
        print('커피 머신을 종료합니다.')
        is_on = False
    # todo - 2 : choice가 report라면 물 : 어쩌고ml ~ 커피 : 어쩌고g / 돈 : $몇달러 라고 출력될 수 있도록 작성하시오. -> def report()를 call1() 유형으로 정의해서 함수만 호출하시오.
        ...
        report()
    # todo - 3: choice가 에스프레소 / 라떼 / 카푸치노에 해당된다면 실행문으로 다음 단계로직 이라고 콘솔에  출력할 수 있도록 코드를 작성하시오.
    ...
    # todo - 4: 오타 발생 시에 잘못 입력하셨습니다를 콘솔에 출력하고 다음 반복으로 넘ㄴ어갈 수 잇도록 코드를 작성하시오.
    else:
        print('잘못 입력하셨습니다.')
