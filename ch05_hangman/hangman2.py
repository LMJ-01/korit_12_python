...
for i in range(len(chosen_word)):
    display.append('_')
print(display)
#todo - 3 : chosen_word의 각 문자들을 반복시켰을 떄 그 위치가 guess와 일치한다면 해당 위치의 display에 문자를 공개합니다. 예를 들어 chosen_word가 apple이고 guess == 'p' 라면 display = [ '_', 'p', 'p', '_', '_' ] 바뀌어야 합니다.

# 특정 index에 있는 '_'를 guess로 대입해줘야 합니다.
# chosen_word[i]가 guess와 일치한다면 display[i]('_'겠죠)를 guess로 재대입해줘야 합니다.
guess = input('알파벳 입력 >>> ').lower()
for i in range(len(chosen_word)):
    if guess == chosen_word[i]: # 일치한다면 해당하는 ==
