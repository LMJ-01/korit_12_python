...
class Car:
    ...
    def add_oil(self, oil):
        ...
        self.oil += oil
        if self.oil > self.max_oil:     # 클래스 변수의 한계값 초과라면
...
class Hybrid(Car):
    ...
    def hybrid_info(self):
        super().car_info()      # 현재 주유 상태가 불러와지겠네요. -> 어차피 부모 객체가 생성됐기 떄문에 super()키워드를 통해 메서드를 호출할 수 있을겁니다. 그래서 저같으면 hybrid_info()라는 별개의 메서드라기 보다는 car_info()메서드로 만든 다음에 재정의했을 것 같습니다.
